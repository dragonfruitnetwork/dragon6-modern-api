...
{
    [TestFixture]
    public class StatsSummaryTests : Dragon6ApiTest
    {
        ...
        public void BasicStatsSummaryTest(string userId, Platform platform, bool expectResults)
        {
            var account = GetAccountFor(userId, platform);
            var summaryContainer = Client.GetModernStatsSummaryFor(account, startDate: DateTimeOffset.Now.AddDays(-30));

            var summary = summaryContainer?.AllModes.AsAny.First();

            if (summary == null && !expectResults)
            {
                Assert.Pass();
            }
            else
            {
                Assert.IsTrue(summary!.MatchesPlayed > 0);
            }
        }
    }
}
