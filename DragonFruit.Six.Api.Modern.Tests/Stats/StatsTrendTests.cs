...
{
    [TestFixture]
    public class StatsTrendTests : Dragon6ApiTest
    {
        ...
        public void BasicStatsTrendTest(string userId, Platform platform, bool expectResults)
        {
            var account = GetAccountFor(userId, platform);
            var statsTrend = Client.GetModernStatsTrendFor(account, startDate: DateTimeOffset.Now.AddDays(-30));

            var allTrends = statsTrend?.AllModes.AsAny.SingleOrDefault();
            Assert.IsTrue(allTrends?.Wl.Any() ?? !expectResults);
        }
    }
}
